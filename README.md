...
- 爆量榜与竞价榜使用公开接口真实数据，无需 API Key。前者计算当日成交额与此前 20 个交易日均额之比，后者核对开盘竞价分时成交额；仅支持当日，竞价在 09:30 后确认。
- 行情分页失败后等待 1 秒、2 秒各重试一次；仍失败时先计算已读取范围。同一页面会话内保留 15 分钟分页断点，点击“继续扫描 / 补全”从失败页续读，成功计算输入复用本地缓存。跨日、断点过期、证券总数变化或分页重复时重建证券列表；刷新或关闭浏览器页面会丢失分页断点。
- 扫描时最多每 500 毫秒更新一次榜单，展示行情覆盖条数、已核对股票数、失败数和待核对数。未全部完成时标记“不完整”，竞价只表示已核对范围内的前 50 名。部分范围内没有入榜股票不代表全市场没有。
- 个股接口连续失败会暂停，保留已计算结果和计算缓存；第一页完全不可用时明确报错，不生成假数据。公开接口可能限流或断连，首次全市场扫描仍可能耗时较长。续读期间行情来自不同获取时刻，不是同一时刻快照。
- 运行 `pnpm test` 验证分页重试、断点过期、失败保留、流式结果、取消请求及竞价完整度，运行 `pnpm build` 生成静态页面。
