...
浏览器标签优先使用项目根目录与 `public` 下的 `favicon.svg` 品牌矢量图标，`favicon.ico` 作为兼容回退；查询参数用于部署后刷新浏览器图标缓存。可运行 `scripts/generate-favicon.ps1` 重新生成 ICO。
...
- 手机端词卡的单词/词组标题使用较紧凑的响应式字号；英式或美式音标缺失时不渲染对应音标行，但仍保留发音按钮供本地音频、在线接口或设备语音使用。
...
- 全局搜索结果不会覆盖正常词库的浏览位置。
- 左侧显示单词列表。
- 单词列表按可视区域虚拟渲染，并保留上下缓冲行；支持方向键、Home、End 和翻页键定位。
- 左侧列表拥有独立纵向滚动区域。
- 右侧显示当前单词详情：
  - 单词
  - 英式音标
...
- 手机端顶部导航压缩为两行：第一行放品牌、内容下拉和练习入口，第二行将八个分类等宽排列并保留发音开关；“新概念英语”在手机端简写为“新概念”，确保所有分类始终可见。重复的顶部条目总数不再显示。
- 手机端不显示重复的词库名称和词库描述；“词表 / 进度 / 搜索 / 详情”合并为单行工具栏，点击搜索时才临时展开搜索框。桌面端始终完整显示词库介绍和搜索区域。
...
- “词表”按钮打开底部抽屉，抽屉复用当前搜索、全局搜索、虚拟滚动和选中逻辑。
...
设备语音朗读速度为 `0.72`；项目音频和在线音频保持其原始速度。
...
练习模式使用与首页一致的米白、墨绿和珊瑚色主题，核心功能包括：
...
- 词条显示在释义上方，释义紧随词条显示。
- 输入正确字母时，顶部词条中的对应字符同步变为珊瑚色；隐藏词条时正确字符会逐字显露。
- 手机端输入框获得焦点、软键盘弹出时启用紧凑布局：隐藏工具栏、音标详情、例句、词库切换和次要操作，只保留词条、释义、字母输入、反馈及提交按钮；键盘收起后恢复完整页面。
- 隐藏或显示当前单词。
- 播放当前单词。
- 播放当前例句。
- 按指定序号跳转。
- 底部使用“分类 + 内容”两级下拉切换词库，并提供上一个、下一个词库快捷按钮；当前词库名称和条目数会单独显示。
- `Esc` 快捷键重新播放当前单词。
- 数字 `1` 快捷键播放当前例句。
- 数字 `2` 快捷键切换隐藏或显示当前单词。
- `Enter` 快捷键提交答案。
...
- 正确位置使用墨绿色文字和下划线，顶部对应词条字符同步使用珊瑚色强调。
- 错误字母立即变为珊瑚红并执行水平振动动画。
- 点击字母区域可以重新获取输入焦点。
- 支持键盘输入、退格和粘贴。
...
- 单词列表使用虚拟滚动，恢复第几千个词或滚到末尾时也只渲染可视区域和缓冲行。
